...
class BigQueryAdapter(BaseAdapter):
    ...
    def drop_relation(self, relation: BigQueryRelation) -> None:
        ...
        dataset = self.connections.dataset(relation.database, relation.schema, conn)
    ...
    def rename_relation(
        ...
        from_table_ref = self.connections.table_ref(
            from_relation.database, from_relation.schema, from_relation.identifier, conn
        )
        ...
        to_table_ref = self.connections.table_ref(
            to_relation.database, to_relation.schema, to_relation.identifier, conn
        )
    ...
    def check_schema_exists(self, database: str, schema: str) -> bool:
        ...
        bigquery_dataset = self.connections.dataset(database, schema, conn)
    ...
    @available.parse(lambda *a, **k: "")
    def execute_model(self, model, materialization, sql_override=None, decorator=None):
    ...
    def get_table_ref_from_relation(self, conn, relation):
        return self.connections.table_ref(
            relation.database, relation.schema, relation.identifier, conn
        )
    ...
    def update_table_description(
        ...
        table_ref = self.connections.table_ref(database, schema, identifier, conn)
    ...
    def load_dataframe(
        ...
        with open(agate_table.original_abspath, "rb") as f:
            job = client.load_table_from_file(
                f, table, rewind=True, job_config=load_config
            )
    ...
    def get_table_options(
        ...
        if (config.get("hours_to_expiration") is not None) and (not temporary):
            expiration = (
                "TIMESTAMP_ADD(CURRENT_TIMESTAMP(), INTERVAL " "{} hour)"
            ).format(config.get("hours_to_expiration"))
            opts["expiration_timestamp"] = expiration
    ...
    def grant_access_to(self, entity, entity_type, role, grant_target_dict):
        ...
        if entity_type == "view":
            entity = self.connections.table_ref(
                entity.database, entity.schema, entity.identifier, conn
            ).to_api_repr()
