...
class RemoteRPCCli(RPCTask[RPCCliParameters]):
    ...
    def set_config(self, config):
        ...
        if issubclass(self.task_type, RemoteManifestMethod):
            ...
            self.real_task = task_type(self.args, self.config, self.manifest)
        else:
            self.real_task = self.task_type(self.args, self.config)
    ...
    def get_rpc_task_cls(self) -> Type[HasCLI]:
        ...
        raise InternalException(
            "No matching handler found for rpc method {} (which={})".format(
                self.args.rpc_method, self.args.which
            )
        )
    ...
    def handle_request(self) -> Result:
        ...
        self.config.cli_vars.update(parse_cli_vars(getattr(self.args, "vars", "{}")))
