...
class ModelRunner(CompileRunner):
    ...
    def print_result_line(self, result):
        description = self.describe_node()
        print_model_result_line(result, description, self.node_index, self.num_nodes)
    ...
    def _materialization_relations(self, result: Any, model) -> List[BaseRelation]:
        if isinstance(result, str):
            deprecations.warn(
                "materialization-return", materialization=model.get_materialization()
            )
...
class RunTask(CompileTask):
    ...
    def after_run(self, adapter, results):
        ...
        database_schema_set: Set[Tuple[Optional[str], str]] = {
            ...
            for r in results
            if r.status not in (NodeStatus.Error, NodeStatus.Fail, NodeStatus.Skipped)
        }
