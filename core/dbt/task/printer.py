...
from dbt.contracts.results import FreshnessStatus, NodeResult, NodeStatus, TestStatus
...
def print_hook_start_line(statement: str, index: int, total: int) -> None:
    ...
    print_fancy_output_line(msg, "RUN", logger.info, index, total, truncate=True)
...
def print_skip_line(
    ...
    print_fancy_output_line(msg, ui.yellow("SKIP"), logger.info, index, num_models)


def print_cancel_line(model) -> None:
    msg = "CANCEL query {}".format(model)
    print_fancy_output_line(msg, ui.red("CANCEL"), logger.error, index=None, total=None)


def get_printable_result(result, success: str, error: str) -> Tuple[str, str, Callable]:
...
def print_model_result_line(result, description: str, index: int, total: int) -> None:
    info, status, logger_fn = get_printable_result(result, "created", "creating")
...
def print_snapshot_result_line(
    ...
    msg = "{info} {description}".format(info=info, description=description, **cfg)
    print_fancy_output_line(msg, status, logger_fn, index, total, result.execution_time)
...
def print_seed_result_line(result, schema_name: str, index: int, total: int):
    ...
    print_fancy_output_line(
        "{info} seed file {schema}.{relation}".format(
            info=info, schema=schema_name, relation=model.alias
        ),
        status,
        logger_fn,
        index,
...
def print_freshness_result_line(result, index: int, total: int) -> None:
    ...
    print_fancy_output_line(
        msg, color(info), logger_fn, index, total, execution_time=result.execution_time
    )
...
def print_skip_caused_by_error(
    ...
    msg = "SKIP relation {}.{} due to ephemeral model error".format(schema, relation)
    print_fancy_output_line(msg, ui.red("ERROR SKIP"), logger.error, index, num_models)
...
def print_run_end_messages(results, keyboard_interrupt: bool = False) -> None:
    ...
    with DbtStatusMessage(), InvocationProcessor():
        print_end_of_run_summary(len(errors), len(warnings), keyboard_interrupt)
