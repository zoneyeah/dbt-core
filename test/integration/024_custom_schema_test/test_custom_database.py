...
class TestOverrideDatabase(DBTIntegrationTest):
    ...
    def test_snowflake_override_generate_db_name(self):
        ...
        self.assertTableDoesExist(
            "SEED", schema=self.unique_schema(), database=self.default_database
        )
        self.assertTableDoesExist(
            "AGG", schema=self.unique_schema(), database=self.default_database
        )
        ...
        self.assertTableDoesExist(
            "VIEW_1", schema=self.unique_schema(), database=self.default_database
        )
        self.assertTableDoesExist(
            "VIEW_2", schema=self.unique_schema(), database=self.alternative_database
        )
        self.assertTableDoesExist(
            "VIEW_3", schema=self.unique_schema(), database=self.alternative_database
        )
