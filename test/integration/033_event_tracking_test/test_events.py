...
class TestEventTracking(DBTIntegrationTest):
    ...
    def run_event_test(
        ...
        project_id = hashlib.md5(self.config.project_name.encode("utf-8")).hexdigest()
        ...
        for (args, kwargs) in track_fn.call_args_list:
            ordered_contexts.append([context.__dict__ for context in kwargs["context"]])

        populated_contexts = []

        for context in expected_contexts:
            if callable(context):
                populated_contexts.append(
                    context(project_id, user_id, invocation_id, version)
                )
            else:
                populated_contexts.append(context)

        self.assertEqual(ordered_contexts, populated_contexts)
    ...
    def build_context(
        self, command, progress, result_type=None, adapter_type="postgres"
    ):
        def populate(project_id, user_id, invocation_id, version):
            return [
                {
                ...
                {"schema": "iglu:com.dbt/platform/jsonschema/1-0-0", "data": ANY},
                {"schema": "iglu:com.dbt/invocation_env/jsonschema/1-0-0", "data": ANY},
            ]
...
class TestEventTrackingSuccess(TestEventTracking):
    ...
    def test__postgres_event_tracking_compile(self):
        expected_calls = [
            call(category="dbt", action="invocation", label="start", context=ANY),
            call(category="dbt", action="load_project", label=ANY, context=ANY),
            call(category="dbt", action="resource_counts", label=ANY, context=ANY),
            call(category="dbt", action="invocation", label="end", context=ANY),
        ]
    ...
    def test__postgres_event_tracking_seed(self):
        ...
        expected_calls = [
            ...
            call(category="dbt", action="load_project", label=ANY, context=ANY),
            call(category="dbt", action="resource_counts", label=ANY, context=ANY),
            call(category="dbt", action="run_model", label=ANY, context=ANY),
            call(category="dbt", action="invocation", label="end", context=ANY),
        ]
    ...
    def test__postgres_event_tracking_tests(self):
        ...
        self.run_event_test(
            ["test"], expected_calls, expected_contexts, expect_pass=False
        )
...
class TestEventTrackingUnableToConnect(TestEventTracking):
    ...
    def profile_config(self):
        return {
            "config": {"send_anonymous_usage_stats": True},
            "test": {
                "outputs": {
                    "default2": {
                        "type": "postgres",
...
class TestEventTrackingSnapshot(TestEventTracking):
    @property
    def project_config(self):
        return {"config-version": 2, "snapshot-paths": ["snapshots"]}
    ...
    def test__postgres_event_tracking_snapshot(self):
        ...
        self.run_event_test(["snapshot"], expected_calls, expected_contexts)
...
class TestEventTrackingCatalogGenerate(TestEventTracking):
    ...
    def test__postgres_event_tracking_catalog_generate(self):
        ...
        self.run_event_test(["docs", "generate"], expected_calls, expected_contexts)
