...
class TestSimpleSnapshotFileSelects(DBTIntegrationTest):
    ...
    def project_config(self):
        return {
            "config-version": 2,
            "data-paths": ["data"],
            "snapshot-paths": ["test-snapshots-select", "test-snapshots-pg"],
            "macro-paths": ["macros"],
        }
...
class TestSimpleSnapshotFilesBigquery(DBTIntegrationTest):
    ...
    def test__bigquery__snapshot_with_new_field(self):
        ...
        # This adds new fields to the source table, and updates the expected
        # snapshot output accordingly
        ...
        self.assertTrue(
            len(expected_cols) > 0, "source table does not exist -- bad test"
        )
        self.assertEqual(
            len(expected_cols),
            len(snapshotted_cols),
            "actual and expected column lengths are different",
        )
...
class TestCrossDBSnapshotFiles(DBTIntegrationTest):
    ...
    def run_snapshot(self):
        return self.run_dbt(
            [
                "snapshot",
                "--vars",
                '{{"target_database": {}}}'.format(self.alternative_database),
            ]
        )
    ...
    def test__snowflake__cross_snapshot(self):
        ...
        self.assertTablesEqual(
            "SNAPSHOT_EXPECTED", "SNAPSHOT_ACTUAL", table_b_db=self.alternative_database
        )
...
class TestSnapshotHardDelete(DBTIntegrationTest):
    ...
    def _snapshot_and_assert_revived(self):
        ...
        for result in revived_records:
            ...
            # invalidated_snapshot_datetime and not in result datetime so set the
            # microseconds to 0
            self.assertGreaterEqual(
