...
class TestRenameRelation(DBTIntegrationTest):
    ...
    @property
    def project_config(self):
        return {"config-version": 2, "source-paths": ["models"]}
    ...
    def test_bigquery_adapter_methods(self):
        ...
        self.run_dbt(
            ["run-operation", "rename_named_relation", "--args", rename_relation_args]
        )
        self.run_dbt()
...
class TestGrantAccess(DBTIntegrationTest):
    ...
    def test_bigquery_adapter_methods(self):
        ...
        self.adapter.grant_access_to(
            ae_entity, ae_entity_type, ae_role, ae_grant_target_dict
        )
        ...
        unexpected_access_entry = AccessEntry(
            ae_role, ae_entity_type, "unexpected@email.com"
        )
