...
class ServerProcess(dbt.flags.MP_CONTEXT.Process):
    def __init__(self, port, profiles_dir, cli_vars=None):
        ...
        super().__init__(
            target=handle_and_check, args=(handle_and_check_args,), name="ServerProcess"
        )
...
class HasRPCServer(DBTIntegrationTest):
    ...
    def run_dbt_with_vars(self, cmd, *args, **kwargs):
        cmd.extend(["--vars", "{{test_run_schema: {}}}".format(self.unique_schema())])
    ...
    def poll_for_result(
        self, request_token, request_id=1, timeout=180, state="success", logs=None
    ):
        start = time.time()
        kwargs = {
    ...
    def wait_for_state(self, state, timestamp, timeout=180, raise_on_timeout=True):
        started = time.time()
        time.sleep(0.5)
...
class TestRPCServerCompileRun(HasRPCServer):
    ...
    def test_compile_sql_postgres(self):
        trivial = self.async_query("compile_sql", "select 1 as id", name="foo").json()
        self.assertSuccessfulCompilationResult(trivial, "select 1 as id")

        ref = self.async_query(
            "compile_sql", 'select * from {{ ref("descendant_model") }}', name="foo"
        ).json()
        ...
        self.assertSuccessfulCompilationResult(
            macro, "select {{ my_macro() }}", compiled_sql="select 1 as id"
        )
        ...
        ephemeral = self.async_query(
            "compile_sql", 'select * from {{ ref("ephemeral_model") }}', name="foo"
        ).json()
        self.assertSuccessfulCompilationResult(
            ephemeral,
    ...
    def test_run_sql_postgres(self):
        ...
        data = self.async_query("run_sql", "select 1 as id", name="foo").json()
        ...
        ephemeral = self.async_query(
            "run_sql", 'select * from {{ ref("ephemeral_model") }}', name="foo"
        ).json()
        self.assertSuccessfulRunResult(
            ephemeral,
    ...
    def test_invalid_requests_postgres(self):
        ...
        data = self.query("xxxxxnotamethodxxxxx", "hi this is not sql").json()
        ...
        error_data = self.assertIsErrorWith(
            data,
            10004,
            "Compilation Error",
            {
        ...
        data = self.async_query("run_sql", "hi this is not sql", name="foo").json()
    ...
    def test_timeout_postgres(self):
        data = self.async_query(
            "run_sql", "select from pg_sleep(5)", name="foo", timeout=1
        ).json()
...
class TestRPCServerProjects(HasRPCServer):
    ...
    def test_compile_project_cli_postgres(self):
        ...
        result = self.async_query(
            "cli_args", cli="compile --models=source:test_source+"
        ).json()
        self.assertHasResults(
            result,
            {"descendant_model", "multi_source_model"},
...
class TestRPCTaskManagement(HasRPCServer):
    ...
    def test_sighup_postgres(self):
        ...
        done_query = self.async_query(
            "compile_sql", "select 1 as id", name="done"
        ).json()
...
class TestRPCServerDeps(HasRPCServer):
    ...
    def test_deps_cli_compilation_postgres(self):
        ...
        # do a dbt deps, wait for the result
        self.assertIsResult(
            self.async_query("cli_args", cli="deps", _poll_timeout=180).json()
        )
