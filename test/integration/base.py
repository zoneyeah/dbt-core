...
class DBTIntegrationTest(unittest.TestCase):
    ...
    def _drop_schemas_sql(self):
        ...
        drop_alternative = (
            ...
            and self.adapter_type not in {"postgres", "redshift"}
    ...
    def run_dbt(
        self, args=None, expect_pass=True, strict=True, parser=True, profiles_dir=True
    ):
    ...
    def run_dbt_and_check(
        self, args=None, strict=True, parser=False, profiles_dir=True
    ):
        log_manager.reset_handlers()
        if args is None:
            args = ["run"]
