...
def test_rpc_seed_threads(project_root, profiles_root, dbt_profile, unique_schema):
    ...
    with querier_ctx as querier:
        ...
        results = querier.async_wait_for_result(querier.cli_args("seed --threads=7"))
...
def test_rpc_seed_include_exclude(
    ...
    with querier_ctx as querier:
        ...
        results = querier.async_wait_for_result(
            querier.cli_args("seed --select=data_1")
        )
        assert len(results["results"]) == 1
        ...
        assert len(results["results"]) == 1
        results = querier.async_wait_for_result(
            querier.cli_args("seed --exclude=data_2")
        )
        assert len(results["results"]) == 1


@pytest.mark.supported("postgres")
def test_rpc_seed_state(project_root, profiles_root, dbt_profile, unique_schema):
    ...
    with querier_ctx as querier:
        ...
        results = querier.async_wait_for_result(querier.seed())
        assert len(results["results"]) == 1
