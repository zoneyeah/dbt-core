...
def test_rpc_basics(project_root, profiles_root, dbt_profile, unique_schema):
...
def test_gc_change_interval(project_root, profiles_root, dbt_profile, unique_schema):
    ...
    with querier_ctx as querier:
        ...
        result = querier.is_result(
            querier.gc(settings=dict(maxsize=1000, reapsize=5, auto_reap_age=0.1))
        )
        ...
        result = querier.is_result(
            querier.gc(settings=dict(maxsize=2, reapsize=5, auto_reap_age=100000))
        )
        for k in ("deleted", "missing", "running"):
            assert k in result
            assert len(result[k]) == 0
...
@pytest.mark.supported("postgres")
def test_get_status(project_root, profiles_root, dbt_profile, unique_schema):
    project = ProjectDefinition(
...
@pytest.mark.supported("postgres")
def test_missing_tag_sighup(project_root, profiles_root, dbt_profile, unique_schema):
    project = ProjectDefinition(
        models={
    ...
    with querier_ctx as querier:
        ...
        project.models[
            "my_docs.md"
        ] = "{% docs asdf %}have a close tag again{% enddocs %}"
        project.write_models(project_root, remove=True)
...
@pytest.mark.supported("postgres")
def test_get_manifest(project_root, profiles_root, dbt_profile, unique_schema):
    project = ProjectDefinition(
        models={
    ...
    with querier_ctx as querier:
        ...
        manifest = result["manifest"]
        assert (
            manifest["nodes"]["model.test.my_model"]["compiled_sql"] == "select 1 as id"
        )
