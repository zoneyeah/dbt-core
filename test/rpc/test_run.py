...
def test_rpc_run_threads(project_root, profiles_root, dbt_profile, unique_schema):
    ...
    with querier_ctx as querier:
        ...
        results = querier.async_wait_for_result(querier.cli_args("run --threads=7"))
        assert_has_threads(results, 7)


@pytest.mark.supported("postgres")
def test_rpc_run_vars(project_root, profiles_root, dbt_profile, unique_schema):
    ...
    with querier_ctx as querier:
        results = querier.async_wait_for_result(
            querier.cli_args('run --vars "{param: 100}"')
        )
...
@pytest.mark.supported("postgres")
def test_rpc_run_vars_compiled(project_root, profiles_root, dbt_profile, unique_schema):
    project = ProjectDefinition(
        models={
    ...
    with querier_ctx as querier:
        results = querier.async_wait_for_result(
            querier.cli_args('run --vars "{materialized_var: table}"')
        )
        assert len(results["results"]) == 1
...
@pytest.mark.supported("postgres")
def test_rpc_run_state_defer(project_root, profiles_root, dbt_profile, unique_schema):
    project = ProjectDefinition(models={"my_model.sql": "select 1 as id"})
    querier_ctx = get_querier(
        project_def=project,
        project_dir=project_root,
    ...
    with querier_ctx as querier:
        ...
        project.models[
            "my_model.sql"
        ] = '{% if execute %}{% do exceptions.raise_compiler_error("should not see this") %}{% endif %}select 2 as id'
