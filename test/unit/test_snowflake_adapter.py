...
class TestSnowflakeAdapter(unittest.TestCase):
    ...
    @mock.patch(
        "dbt.adapters.snowflake.SnowflakeCredentials._get_private_key",
    ...
    def test_authenticator_private_key_authentication_no_passphrase(
...
class TestSnowflakeAdapterConversions(TestAdapterConversions):
    ...
    def test_convert_datetime_type(self):
        ...
        for col_idx, expect in enumerate(expected):
            assert (
                SnowflakeAdapter.convert_datetime_type(agate_table, col_idx) == expect
            )
...
class SnowflakeConnectionsTest(unittest.TestCase):
    def test_comment_stripping_regex(self):
        ...
        stripped_comment1 = re.sub(
            re.compile(pattern, re.MULTILINE), "", comment1
        ).strip()

        stripped_comment2 = re.sub(
            re.compile(pattern, re.MULTILINE), "", comment2
        ).strip()
