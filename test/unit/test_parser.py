...
class SchemaParserSourceTest(SchemaParserTest):
    ...
    def test__parse_basic_source_tests(self):
        ...
        self.assertIn(path, self.parser.results.files)
        self.assertEqual(
            self.parser.results.files[path].sources,
...
class SchemaParserModelsTest(SchemaParserTest):
    ...
    def test__parse_basic_model_tests(self):
        ...
        self.assertEqual(tests[0].fqn, ["snowplow", "schema_test", tests[0].name])
        self.assertEqual(
            tests[0].unique_id.split("."), ["test", "snowplow", tests[0].name]
        )
        ...
        self.assertEqual(
            tests[1].unique_id.split("."), ["test", "snowplow", tests[1].name]
        )
        ...
        self.assertEqual(
            tests[2].unique_id.split("."), ["test", "snowplow", tests[2].name]
        )
        ...
        self.assertIn(path, self.parser.results.files)
        self.assertEqual(
            sorted(self.parser.results.files[path].nodes), [t.unique_id for t in tests]
        )
        self.assertIn(path, self.parser.results.files)
...
class ModelParserTest(BaseParserTest):
    ...
    def test_basic(self):
        ...
        self.assertIn(path, self.parser.results.files)
        self.assertEqual(
            self.parser.results.files[path].nodes, ["model.snowplow.model_1"]
        )
...
class SnapshotParserTest(BaseParserTest):
    ...
    def test_single_block(self):
        ...
        self.assertIn(path, self.parser.results.files)
        self.assertEqual(
            self.parser.results.files[path].nodes, ["snapshot.snowplow.foo"]
        )
    ...
    def test_multi_block(self):
        ...
        self.assertIn(path, self.parser.results.files)
        self.assertEqual(
            sorted(self.parser.results.files[path].nodes),
...
class DataTestParserTest(BaseParserTest):
    ...
    def test_basic(self):
        ...
        self.assertIn(path, self.parser.results.files)
        self.assertEqual(
            self.parser.results.files[path].nodes, ["test.snowplow.test_1"]
        )
...
class AnalysisParserTest(BaseParserTest):
    ...
    def test_basic(self):
        ...
        self.assertIn(path, self.parser.results.files)
        self.assertEqual(
            self.parser.results.files[path].nodes, ["analysis.snowplow.analysis_1"]
        )
...
class ProcessingTest(BaseParserTest):
    ...
    def test_process_sources(self):
        ...
        self.x_node.depends_on.nodes.append.assert_called_once_with(
            "source.thirdproject.src.tbl"
        )
