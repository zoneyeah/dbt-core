...
from dbt.context import docs, macros, manifest, providers, target
...
def test_base_context():
    ctx = context.generate_base_context({})
...
def test_resolve_specific(
    config, manifest_extended, redshift_adapter, get_include_paths
):
    ...
    assert (
        ctx["adapter"].dispatch("some_macro", packages=["root"]).macro
        is package_rs_macro
    )
    assert (
        ctx["adapter"].dispatch("some_macro", packages=["root", "dbt"]).macro
        is package_rs_macro
    )
    assert (
        ctx["adapter"].dispatch("some_macro", packages=["dbt", "root"]).macro
        is rs_macro
    )


def test_resolve_default(
    config, manifest_extended, postgres_adapter, get_include_paths
):
    ...
    assert (
        ctx["adapter"].dispatch("some_macro", packages=["root"]).macro is package_macro
    )
    assert (
        ctx["adapter"].dispatch("some_macro", packages=["root", "dbt"]).macro
        is package_macro
